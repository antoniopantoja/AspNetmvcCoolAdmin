...
function Sucesso(data) {
    if (data)
        $("#sucesso .textoMsgModal").html(data.out_msg);
    else
        $("#sucesso .textoMsgModal").html("Operação Realizada com Sucesso!");
...
function Erro(data) {
    if (data)
        ...
        $("#erro .textoMsgModal").html("FALHA NA OPERAÇÃO!");
...
$.fn.extend({
    //ACEITA SOMENTE NÚMEROS NO CAMPO
    apenasNumeros: function () {
        return this.each(function () {
            $(this).keypress(function (event) {
                var charCode = (event.which) ? event.which : event.keyCode
                return charCode == 8 || charCode == 9 || (charCode > 47 && charCode < 58);
            });
        });
    }
});
